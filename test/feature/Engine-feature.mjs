...
Feature('Engine', () => {
  Scenario('Mother of all', () => {
    let engine, source;
    Given(
      'a massive source with user task, sub process, lanes, and a loop back',
      () => {
    ...
    Then('definitions has preset settings and variables', () => {
      for (const def of api.definitions) {
        expect(def.environment.settings).to.have.property(
          'someSetting',
          'preset'
        );
    ...
    And('definition have iterated completed counter', () => {
      expect(api.definitions[0])
        .to.have.property('counters')
        .with.property('completed', 1);
    });
