  ...
  describe('issue 19 - save state', () => {
    it('make sure there is something to save on activity start event', async () => {
      ...
      for (const state of states) {
        ...
        expect(state.definitions[0].execution)
          .to.have.property('processes')
          .with.length(1);
        expect(state.definitions[0].execution.processes[0]).to.have.property(
          'execution'
        );
        expect(state.definitions[0].execution.processes[0].execution)
          .to.have.property('children')
          .with.length(7);

        const children =
          state.definitions[0].execution.processes[0].execution.children;

        expect(children.map(({ id }) => id)).to.eql([
          'Start',
          'Parallel1',
          'Task_A',
          'Task_B',
      ...
      let [Start, Parallel1, Task_A, Task_B, Parallel2, Task_C, End] =
        states[0].definitions[0].execution.processes[0].execution.children;
      expect(Start, 'state 0 Start').to.have.property('status', 'started');
      expect(Parallel1, 'state 0 Parallel1').to.not.have.property('status');
      [Start, Parallel1, Task_A, Task_B, Parallel2, Task_C, End] =
        states[1].definitions[0].execution.processes[0].execution.children;
      expect(Start, 'state 1 Start').to.have.property('status', 'end');
      expect(Parallel1, 'state 1 Parallel1').to.have.property(
        'status',
        'started'
      );
      expect(Task_A, 'state 1 Task_A').to.not.have.property('status');
      [Start, Parallel1, Task_A, Task_B, Parallel2, Task_C, End] =
        states[2].definitions[0].execution.processes[0].execution.children;
      ...
      [Start, Parallel1, Task_A, Task_B, Parallel2, Task_C, End] =
        states[3].definitions[0].execution.processes[0].execution.children;
      ...
      [Start, Parallel1, Task_A, Task_B, Parallel2, Task_C, End] =
        states[4].definitions[0].execution.processes[0].execution.children;
      expect(Task_A, 'state 4 Task_A').to.not.have.property('status');
      expect(Task_B, 'state 4 Task_B').to.have.property('status', 'end');
      expect(Parallel2, 'state 4 Parallel2').to.have.property(
        'status',
        'started'
      );
      [Start, Parallel1, Task_A, Task_B, Parallel2, Task_C, End] =
        states[5].definitions[0].execution.processes[0].execution.children;
      ...
      [Start, Parallel1, Task_A, Task_B, Parallel2, Task_C, End] =
        states[6].definitions[0].execution.processes[0].execution.children;
  ...
  describe('issue 74 - setting engine output from script', () => {
    ...
    it('engine output is not altered during execution', async () => {
      ...
      listener.on('activity.wait', (api) => {
        ...
        expect(api.environment.output.test, 'execution output').to.equal(
          'set from script'
        );
