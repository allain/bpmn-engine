...
// import { deserialize, TypeResolver } from "moddle-context-serializer";
...
Engine.prototype.getDefinitions = async function getDefinitions(
  executeOptions
) {
  const loadedDefinitions = this[kLoadedDefinitions];
...
Engine.prototype._loadDefinitions = async function loadDefinitions(
  executeOptions
) {
...
Engine.prototype._loadDefinition = function loadDefinition(
  serializedContext,
  executeOptions = {}
) {
...
Engine.prototype._serializeModdleContext = function serializeModdleContext(
  moddleContext
) {
...
Execution.prototype._addConsumerCallbacks = function addConsumerCallbacks(
  callback
) {
  if (!callback) return;
...
Execution.prototype._teardownDefinition = function teardownDefinition(
  definition
) {
  const executing = this[kExecuting];
...
Execution.prototype.signal = function signal(
  payload,
