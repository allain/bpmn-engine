...
describe('Engine', () => {
  ...
  describe('async getDefinitionById()', () => {
    it('returns definition of passed moddle context', async () => {
      const moddleContext = await testHelpers.moddleContext(
        factory.valid('contextTest')
      );
    ...
    it('returns definition of passed deserialized moddle context', async () => {
      ...
      const engine = new Bpmn.Engine({
        moddleContext: JSON.parse(
          JSON.stringify(testHelpers.serializeModdleContext(moddleContext))
        )
      });
  ...
  describe('async execute([options, callback])', () => {
    ...
    it('with options runs definitions with options', (done) => {
      ...
      engine.once('end', (execution) => {
        expect(execution.definitions[0].environment.variables).to.have.property(
          'input',
          1
        );
    ...
    it('execute options overrides engine options', (done) => {
      ...
      engine.execute({
        ...
        services: {
          get(context, next) {
            if (context.environment.variables.input !== 1) {
              return next(new Error('Get error'));
            }
    ...
    it('writes to environment output on end', async () => {
      ...
      listener.on('wait', (activityApi) => {
        expect(activityApi)
          ...
          .with.property('ioSpecification')
          .with.property('dataInputs')
          .with.length(1);
        expect(
          activityApi.content.ioSpecification.dataInputs[0]
        ).to.have.property('value', 'von Rosén');
        expect(activityApi.content.ioSpecification)
          .to.have.property('dataOutputs')
          .with.length(1);
        expect(
          activityApi.content.ioSpecification.dataOutputs[0]
        ).to.have.property('id', 'userInput');
      ...
      expect(engine.environment.output)
        ...
        .with.property('inputFromUser', 'von Rosen');
    });
  });
  ...
  describe('waitFor(eventName)', () => {
    ...
    it('mandatory non-error messages are ignored', async () => {
      ...
      engine.broker.publish(
        'event',
        'engine.error',
        {},
        { mandatory: true, type: 'fatal' }
      );
  ...
  describe('getState()', () => {
    ...
    it('engine returns non-running state', async () => {
      const sourceEngine = new Bpmn.Engine({
        name: 'test state',
        source: factory.userTask()
      });
  ...
  describe('recover(state[, recoverOptions])', () => {
    it('recovers engine from state', async () => {
      ...
      const engine = new Bpmn.Engine().recover(await sourceEngine.getState());
    ...
    it('recover without state is simply ignored', async () => {
      const engine = new Bpmn.Engine().recover();
    ...
    it('doesn´t overwrite name from state if instantiated with name', async () => {
      ...
      const engine = new Bpmn.Engine({ name: 'my new name' }).recover(
        await sourceEngine.getState()
      );
    ...
    it('environment is recovered', async () => {
      ...
      const recovered = new Bpmn.Engine().recover(await engine.getState());

      expect(recovered.environment.variables).to.have.property(
        'execVersion',
        1
      );
    });
    ...
    it('recover options initializes new environment and passes options to recovered definitions', async () => {
      ...
      expect(definition.environment.variables).to.have.property(
        'execVersion',
        1
      );
      expect(definition.environment.variables).to.have.property(
      ...
      expect(definition.environment.services)
  ...
  describe('dataObjects', () => {
    it('adds dataObject values to output', (done) => {
      ...
      listener.on('wait', (userTask) => {
        expect(userTask.content)
          .to.have.property('ioSpecification')
          .with.property('dataOutputs')
          .with.length(1);
        userTask.signal({
          ioSpecification: {
            dataOutputs: [
